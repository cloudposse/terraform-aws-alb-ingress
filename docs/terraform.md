...
| <a name="input_protocol_version"></a> [protocol\_version](#input\_protocol\_version) | The protocol\_version for the created ALB target group (if `target_group_arn` is not set and if protocol is either HTTP or HTTPS) | `string` | `"HTTP1"` | no |
