...
| <a name="input_content_type"></a> [content\_type](#input\_content\_type) | Content type (for fixed_response action). Supported values are `text/plain`, `text/css`, `text/html`, `application/javascript` and `application/json` | `string` | `""` | no |
...
| <a name="input_message_body"></a> [message\_body](#input\_message\_body) | The message body (for fixed_response action). | `string` | `null` | no |
...
| <a name="input_status_code"></a> [status\_code](#input\_status\_code) | The HTTP response code (for fixed_response action). Valid values are 2XX, 4XX, or 5XX. | `string` | `null` | no |
